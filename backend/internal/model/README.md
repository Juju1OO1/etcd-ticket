...
| 欄位      | 型別      | db tag     | 說明                              |
|-----------|-----------|------------|-----------------------------------|
| ID        | int64     | id         | 自動遞增主鍵（DB 產生，不需傳入）|
| OrderID   | string    | order_id   | UUID 唯一識別碼，由 PublishOrder 產生 |
...
| CreatedAt | time.Time | created_at | 建立時間（DB 產生，不需傳入）     |

使用方：
- `repository.InsertOrder()` — 寫入
- `repository.GetOrdersByUser()` — 查詢
